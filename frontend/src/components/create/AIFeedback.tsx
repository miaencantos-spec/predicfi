...
interface AIFeedbackProps {
  ...
  aiFeedback: { valid: boolean; reason: string; improved_question?: string } | null;
}
...
export function AIFeedback({ isValidating, aiFeedback }: AIFeedbackProps) {
  ...
  return (
    <div className={cn(
      ...
      aiFeedback?.valid ? "bg-emerald-50 border-emerald-100" : "bg-red-50 border-red-100"
    )}>
      <div className="flex items-center gap-4 mb-4">
        <div className={cn(
          "p-3 rounded-2xl",
          isValidating ? "bg-zinc-200" : aiFeedback?.valid ? "bg-emerald-200" : "bg-red-200"
        )}>
          {isValidating ? <Sparkles className="w-6 h-6 text-zinc-400 animate-spin" /> : 
           aiFeedback?.valid ? <Check className="w-6 h-6 text-emerald-600" /> : <BrainCircuit className="w-6 h-6 text-red-600" />}
        </div>
        <div>
          <h4 className="font-black text-xs uppercase tracking-widest text-zinc-900">
            {isValidating ? "Validando con IA..." : aiFeedback?.valid ? "Propuesta Validada" : "Revisión Sugerida"}
          </h4>
